...
//#include <cstdlib>
using namespace std;
const int number_of_particles = 36, u=sqrt(number_of_particles), step = 1000, NTIME=1000000, control_temp=1;
int o = 0;
...
int stop = 0;
double R, dx, dy;
const int control_point_1=100000, control_point_2=500000, control_point_3=1000000;
...
int main() 
{
ofstream write, kinwrite, potwrite, fullwrite, tempwrite, avtempwrite, partwrite, rwrite; 
write.open("data.txt");
...
// KIN ENERGY AND TEMPETATURE
	...
	for (int i=0; i<number_of_particles; i++)
	{
		...
		dx+=coord[i][N].x-coord[i][N-1].x;//velocity[i][N].x*dt+0.5*acceleration[i][N].x*pow(dt,2);
		dy+=coord[i][N].y-coord[i][N-1].y;//velocity[i][N].y*dt+0.5*acceleration[i][N].y*pow(dt,2);
		R+= pow(dx,2)+pow(dy,2);
	}
	...
	podgonian=kin_energy[N]+pot_energy[N]-kin_energy[N-1]-pot_energy[N-1];
	if (N<control_point_1 or (N>control_point_1+100 and N<control_point_2) or (N>control_point_2+100 and N<control_point_3))
	{
		//pot_energy[N]=pot_energy[N]-podgonian;
	}
