...
const double  L_x=5, L_y=sqrt(3)*L_x/2, a=L_x/(n_c-1), h = sqrt(3)*a/2;
const int step = 1000; //skip 1000 coordinates
const int NTIME=10 /* number of steps */, control_temp=1 /* T control enable/disable 1/0 */;
int o = 0; //T control steps
double length[number_of_particles][number_of_particles];
const double sigma=2.74, epsilon=0.0031, dt=0.0001, primary_velocity = 1;
...
int main() {
    ...
    for (int i=0; i<n_c; i++){
		for (int j=0; j<n_c; j++){
		coord[i*n_c+j][0].x=j*(0.9*30)/(n_c-1);//j*a;
		coord[i*n_c+j][0].y=i*(0.9*30)/(n_c-1);//i*h;
...
//COORDINATES 0
	for (int i=0; i<n_c;  i++)
	{
		for (int j=0; j<n_c; j++)
		{
			coord[i*n_c+j][0].x=a*j;
			coord[i*n_c+j][0].y=h*i;
		}
	}
	for (int i=0; i<number_of_particles; i++)
	{
	...
	for (int i=0; i<number_of_particles; i++) //CHOOSE A PARTICLE TO FIND A DISTANCE
	{
			for ( int j=0; j<number_of_particles; j++)
			{
				...
				//cout << "length_" << i << "_" << j << " = " << length[i][j] << endl;
			}
	}
	for (int i=0; i<number_of_particles; i++) 
	{
	...
	for (int i=0; i<number_of_particles; i++)
	{
		for (int j=0; j<number_of_particles; j++)
		{
			...
			//cout << "acceleration_x_" << i << "_" << j <<  " = " << acceleration[i][0].x << endl;	
			//cout << "acceleration_y_" << i << "_" << 0 <<  " = " << acceleration[i][0].y << endl;
		}
	...
	for (int i=0; i<number_of_particles; i++)
	{
		...
		//cout << "" << endl;
		//cout << "" << endl;
	cout << 0 << " kin_energy " << kin_energy[0] << endl;
	cout << 0 << " pot_energy " <<  pot_energy[0] << endl; 
	cout << 0  << " total energy " << kin_energy[0]+pot_energy[0] << endl; 
...
// KIN ENERGY AND TEMPETATURE
	...
	for (int i=0; i<number_of_particles; i++)
	{
		...
		if (abs(coord[i][N+1].x)>L_x or abs(coord[i][N+1].y)>L_y or kin_energy[N]-kin_energy[N-1]>10 or acceleration[i][N].x-acceleration[i][N-1].x>1 or acceleration[i][N].y-acceleration[i][N-1].y>1 )
		{
			...
			{
			cout <<  coord[i][N].x << " " << coord[i][N].y << endl;
			if (i==14)
			{
	...
	cout << N << " " << kin_energy[N] << endl;
	cout <<  N << " " <<  pot_energy[N] << endl; 
	cout << N  << " " << kin_energy[N]+pot_energy[N] << endl;  
	cout << N << " " << temperature_average << endl;
